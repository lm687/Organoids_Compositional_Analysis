---
title: "OV cell lines (Maria Vias, Brenton group)"
author: "L Morrill"
date: "2019"
output:
    html_document:
...
```{r,fig.height=4.5,messages=FALSE, echo=FALSE,warning=FALSE, cache=TRUE}
...
```{r, natgen_data,include=FALSE, cache=TRUE}
natgen <- list()
...
```{r, clr_funs,include=FALSE,eval=TRUE, cache=TRUE}
clr_vec <- function(x){
  log(x) - mean(log(x))
}
clr_mat <- function(X){
...
```{r, clr,include=FALSE,eval=TRUE, cache=TRUE}
...
```{r, cols,include=FALSE, cache=TRUE}
n <- 60
...
```{r, princomp,echo=FALSE, cache=TRUE}
...
```{r, pca_from_scratch,include=TRUE, echo=FALSE, cache=TRUE}
...
```{r, loadings, fig.height=4, echo=FALSE, cache=TRUE}
...
```{r, dendrogram_aitchisondistance,echo=FALSE, cache=TRUE}
par(mfrow=c(1,2))

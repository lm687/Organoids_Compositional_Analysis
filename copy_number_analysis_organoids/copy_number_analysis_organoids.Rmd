...
dend_data$labels$label[!grepl('Sample', dend_data$labels$label)] = ""
p <- ggplot(dend_data$segments) + 
  geom_segment(aes(x = x, y = y, xend = xend, yend = yend))+
  # geom_text(data = dend_data$labels, aes(x, y, label = label),
  #           hjust = 1, angle = 90, size = 3)+
  geom_label_repel(data = dend_data$labels, aes(x, y, label = label),
            hjust = 1, angle = 90, size = 3, vjust=0)+
  ylim(-3, 15)+
  theme(axis.title.x=element_blank(),
    axis.text.x=element_blank(),
    axis.ticks.x=element_blank(),
...
plot_rank = function(df_rank){
  ...
  ggplot(df_rank, aes(x=factor(Var1, as.character(Var1)[order(value)]),
    ...
    lims(y=c(-max(df_rank$value)*0.3, 2+max(df_rank$value)))+
         # x=c(0, 500))+  #c(-nrow(df_rank)*0.3, nrow(df_rank)*1.3))+
...
pdf("figures/rank_segments.pdf", width = 14, height = 6)
plot_rank(melt(number_of_segments))+ggtitle("Number of segments")
dev.off()
pdf("figures/rank_ploidy.pdf", width = 14, height = 6)
plot_rank(cbind.data.frame(value=as.numeric(ploidies$ploidy), L1=gsub("ploidy_", "", as.character(ploidies$cohort)),
                           Var1=rownames(ploidies)))+ggtitle("Ploidy")
dev.off()

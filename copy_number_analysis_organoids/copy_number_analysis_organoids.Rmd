...
- version of the TCGA etc signatures: the new exposures (sent by Ruben on late November 2020) I just have for TCGA, but this is because the ASCAT segments have been modified differently, and it shouldn't make a difference for non-SNP array.
...
```{r, barplot_organoids2, fig.height=2.5,message=FALSE, echo=FALSE,warning=FALSE}
createBarplot(rename_rows(org, gsub('Sample ', 'PDO', rownames(org))), remove_labels = FALSE, order_labels = gsub('Sample ', 'PDO', names(sort(org[,4])))) + 
    scale_fill_brewer(palette="Dark2")+labs(y='Exposure')+
  ggtitle('Exposures for the organoids')+labs(x='')+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
grid.arrange(give_pca(org, title='Untransformed, centered', names=rownames(org),
                      short_pdo_names=T)+theme(plot.title = element_text(size=7)),
...
give_pca(as(compositions::clr(org), 'matrix'), title='CLR, centered', names=rownames(org),
                      short_pdo_names=T)+theme(plot.title = element_text(size=7)),
give_pca(as(compositions::clr(impute(org, 1e-2)), 'matrix'), title='CLR+imp, centered', names=rownames(org),
                      short_pdo_names=T)+theme(plot.title = element_text(size=7)), ncol=3)
```
...
give_pca(as(compositions::clr(org), 'matrix'), title='CLR, centered', names=rownames(org), give_loadings = T),
give_pca(as(compositions::clr(impute(org, 1e-2)), 'matrix'), title='CLR+imp, centered', names=rownames(org), give_loadings = T), ncol=3)
```
...
```{r, barplot_organoids_alr, fig.height=3.5, fig.width=5, message=FALSE, echo=FALSE,warning=FALSE}
createBarplot((org/org[,1])[,-1], remove_labels = TRUE, order_labels = names(sort(org[,1]))) + 
    labs(y='ALR with s1 baseline')+facet_wrap(.~Var2, ncol=3, scales='free_y')+
  ggtitle('ALR with s1 as baseline')+theme_bw()
```
...
General trends in differences between organoids and other samples
```{r, s5_average}
# head(all_natgen[[which_natgen]][grepl('PDO', all_natgen[[which_natgen]]),])
# head(all_natgen[[which_natgen]][!grepl('PDO', all_natgen[[which_natgen]]),])
...
```{r, proportion_differences_org_primary}
## looks like natgen[[which_natgen]] already doesn't include any organoids here
# natgen[[which_natgen]][grepl('^Sample ', rownames(natgen[[which_natgen]])),]
# head(natgen[[which_natgen]])
# table(natgen_metadata[[which_natgen]]$study)
# head(natgen_metadata[[which_natgen]])
boxplot(list(all=remove_organoids(natgen[[which_natgen]])[,'s5'],
             orgs=org[,'s5']))
```
...
```{r, raw_hclust, dependson=c('dendrogram_gen_impute', 'hierarchical_compositional'), echo=F}
extra_expand <- 0.08
...
```{r, raw_hclust_lowerinput, dependson=c('dendrogram_gen_impute', 'hierarchical_compositional'), echo=F, warning=F, message=F}

# aaaaaa <- give_dendrogram_from_imputation(1/100, plot_dendro=F, return_grob=T, plot=T)
# cowplot::plot_grid(aaaaaa)
cowplot::plot_grid(give_dendrogram_from_imputation(1/250, plot_dendro=F, return_grob=T),
                   give_dendrogram_from_imputation(1/100, plot_dendro=F, return_grob=T))

# cowplot::as_grob(give_dendrogram_from_imputation(1/100, plot_dendro=F, return_grob=T))

# grid.arrange(give_dendrogram_from_imputation(1/100, plot_dendro=F, return_grob=T),
#              give_dendrogram_from_imputation(1/250, plot_dendro=F, return_grob=T))

saveRDS(all_natgen[[which_natgen]], file = paste0("robjects/allnatgen_", which_natgen, ".RDS"))

```

What is the extent of heterogeneity across the PDOs?
```{r, heterogeneity_in_cluster, fig.width=4}

remove_labels_tree <- function(hclust_obj){
  hclust_obj$labels[!grepl("PDO", hclust_obj$labels)] = ''
  # hclust_obj$labels[grepl("PDO", hclust_obj$labels)] = '*'
  return(hclust_obj)
}
tree_inputclrmin2 <- remove_labels_tree(hclust(dist(as(compositions::clr(impute(all_natgen[[which_natgen]], 1e-2)), 'matrix'))))
plot(tree_inputclrmin2)

number_subtrees_with_organoids <- function(k){
  cutree_k <- cutree(tree_inputclrmin2, k=k)
  cutree_k_org <- cutree_k[grepl('PDO', tree_inputclrmin2$label)] ## selecting the organoids
  length(unique(cutree_k_org))
}

organoids_in_subtrees <- function(k){
  cutree_k <- cutree(tree_inputclrmin2, k=k)
  sapply(1:k, function(j) gtools::mixedsort(python_like_select(names(cutree_k)[(cutree_k == j)], 'PDO')))
}

organoids_in_subtrees(3)

plot(1:20, sapply(1:20, number_subtrees_with_organoids), xlab='Number of clades',
     ylab='Number of clades with organoids', xlim=c(1,20), ylim=c(1,20), type='l')
abline(coef = c(0,1), lty='dashed')

plot(1:20, sapply(1:20, number_subtrees_with_organoids)/(1:20), xlab='Number of clades',
     ylab='Fraction of clades with organoids', xlim=c(1,20), ylim=c(0,1), type='l')

organoids_in_subtrees(20)

# xxxxx <- hclust(dist(t(heatmap_dendrogram_df_inputclr)))
# # xxxxx$labels = ''
# xxxxx$labels 
# plot(xxxxx)
```

```{r, pca_for_two_groups}
...
```{r, tanglegram, eval=FALSE, dependson='hierarchical_compositional', echo=FALSE}
...
tanglegram(dend1comparison, dend2comparison, cex_main=.5)
tanglegram(dend1comparison, dend3comparison, cex_main=.5)
tanglegram(dend1comparison, dend4comparison, cex_main=.5)
tanglegram(dend2comparison, dend4comparison, cex_main=.5)
```
...
```{r, s3-s4_pairs, fig.height=2, fig.width=2}
...
```{r, sig_pairs_2, fig.height=2, fig.width=4}
...
The fraction of zero exposures in the s3-rich clade is
- lower in s1 (stat.signif.; p-value = 3.159e-07)
- lower in s3 (stat.signif.; p-value < 2.2e-16)
- higher in s4 (stat.signif.; p-value < 2.2e-16)
- higher in s6 (stat.signif.; but marginally)
- higher in s7 (stat.signif.; p-value = 1.409e-05)

```{r, sig_pairs_3, fig.height=2, fig.width=4}
...
```{r, s2_and_s4_and_s6_organoids, fig.height=2, fig.width=4, warning=F}
...
```{r, save_renamed_organoid_features}
saveRDS(organoids_absolute_copynumber, "data/absolute_CN/absoluteCN_organoid.RDS")
```
...
```{r, nsegments_pcawg_plot_save, dependson=c('nsegments_pcawg_plot')}
saveRDS(nsegments_pcawg_plot, "robjects/nsegments_pcawg_plot.RDS")
```
...
```{r, rank_plots, fig.width=10, echo=FALSE, fig.height=3, fig.width=5, warning=F}
...
plot_rank(melt(number_of_segments), nudge_scalar=1, size_labels=3)+ggtitle("Number of segments")
plot_rank(cbind.data.frame(value=as.numeric(ploidies$ploidy),
                           L1=gsub("ploidy_", "", as.character(ploidies$cohort)),
                           Var1=rownames(ploidies)),
          size_labels=3, nudge_scalar=0.2)+
  ggtitle("Ploidy")
...
```{r, annotate_genes, eval=FALSE, echo=F}
...
```{r, compare_amplification, dependson=c('annotate_genes'), eval=FALSE, echo=F}
# data("geneInfo")
...
```{r, heatmap_ampldel_organoids, dependson=c('compare_amplification_generalised'), eval=FALSE, echo=F}
...
```{r, heatmap_ampldel_organoids_v2, eval=FALSE, dependson=c('compare_amplification_generalised'), echo=F}
...
```{r, heatmap_ampldel_organoids_v2_b, dependson=c('heatmap_ampldel_organoids_v2'), eval=FALSE, echo=F}
...
```{r, correlation_rnaseq_absoluteCN, eval=FALSE, echo=F}
...
## Ascites
```{r, ascites_barplot, fig.height=3, fig.width=4, echo=F}
...
renaming_patients <- read.table("../RNASeq_and_CN/20191218_ViasM_BJ_orgaBrs/Input/samples.csv",
                                sep = ";", header = T)
...
ggplot(ascites_df, aes(x=bool_ascites, y=value, fill=variable))+geom_bar(stat = "identity")+
...
## Observations about specific orgnaoids
```{r, quiet_organoids}
...
```{r, cn_regions_subclonal, echo=FALSE, eval=T}
...
```{r, cn_regions_subclonal_individual, echo=FALSE, eval=T}
...
### Additional plots
```{r, active_sigs_ploidy, fig.width=4, fig.height=4}
org
ploidy_organoids
sort(ploidy_organoids)
quantile(org, c(0.05, 0.1))
apply(org, 1, function(i) mean(i==0))[paste0('PDO', 1:18)]
ploidy_organoids[paste0('PDO', 1:18)]

table(ploidy_organoids[paste0('PDO', 1:18)] < 2.5,
      apply(org, 1, function(i) mean(i==0))[paste0('PDO', 1:18)] == 0)

par(mfrow=c(1,2))
plot(ploidy_organoids[paste0('PDO', 1:18)],
      apply(org, 1, function(i) mean(i==0))[paste0('PDO', 1:18)])

plot(ploidy_organoids[paste0('PDO', 1:18)] < 2.5,
      apply(org, 1, function(i) mean(i==0))[paste0('PDO', 1:18)] == 0)

df_ploidy_heterogeneity <- cbind.data.frame(ploidy=ploidy_organoids[paste0('PDO', 1:18)],
                        fraction_inactive_sigs=7*apply(org, 1, function(i) mean(i==0))[paste0('PDO', 1:18)],
                        PDO=paste0('PDO', 1:18))
ggplot(df_ploidy_heterogeneity,
       aes(x=ploidy, y=fraction_inactive_sigs, label=PDO, group=1))+
  geom_point()+geom_smooth(method='lm')+geom_label_repel()+labs(x='Ploidy', y='Number of inactive signatures')


lm_ploidy_heterogeneity <- lm( (apply(org, 1, function(i) mean(i==0))[paste0('PDO', 1:18)]) ~ ploidy_organoids[paste0('PDO', 1:18)] )
summary(lm_ploidy_heterogeneity)

summary(lm(fraction_inactive_sigs~ploidy, data = df_ploidy_heterogeneity))

```

```{r}
names(df_features)
# df_features$`Segment size`
# sapply(organoids_CN_features, length)
num_segments_organoids <- table(organoids_CN_features$segsize$ID)
num_segments_organoids
ploidy_organoids

sensitive <- c('PDO1', 'PDO4', 'PDO3', 'PDO2', 'PDO13', 'PDO12', 'PDO11')
resistant <- c('PDO14', 'PDO8', 'PDO10', 'PDO7', 'PDO9', 'PDO6', 'PDO5')

boxplot(list(as.vector(num_segments_organoids[sensitive]), as.vector(num_segments_organoids[resistant])))
boxplot(list(as.vector(ploidy_organoids[sensitive]), as.vector(ploidy_organoids[resistant])))
num_segments_organoids[sensitive]
num_segments_organoids[resistant]
ploidy_organoids[sensitive]
ploidy_organoids[resistant]

t.test(as.vector(num_segments_organoids[sensitive]), as.vector(num_segments_organoids[resistant]))
t.test(as.vector(ploidy_organoids[sensitive]), as.vector(ploidy_organoids[resistant]))

saveRDS(num_segments_organoids, "robjects/num_segments_organoids.RDS")
saveRDS(ploidy_organoids, "robjects/ploidy_organoids.RDS")

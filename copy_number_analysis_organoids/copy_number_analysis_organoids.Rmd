...
source(paste0(Sys.readlink(paths = "BriTROC-cnsignatures-symlink"), "/main_functions.R"))
...
```{r, heatmap_dendrogram, echo=FALSE}
...
give_pca(as(compositions::clr(org), 'matrix'), title='CLR, centered', names=rownames(org)), ncol=3)
```
...
```{r, hierarchical_compositional}

give_dendrogram_generalised = function(df, modify_labels=T){
  x = hclust(dist(df))
  if(modify_labels){
    x$labels[!grepl("Sample", x$labels)] = ''
    x$labels[grepl("Sample", x$labels)] = '*'
  }
  y = x
...
par(mfrow=c(1,2))
dendroraw = give_dendrogram_generalised(all_natgen[[which_natgen]])
dendroclr = give_dendrogram_generalised(as(compositions::clr(all_natgen[[which_natgen]]), 'matrix'))
par(mfrow=c(1,4), mar=c(0.2,0.2,0.2,0.2))
dendroraw_pdo = give_dendrogram_generalised(all_natgen[[which_natgen]][grepl('PDO', rownames(all_natgen[[which_natgen]])),], modify_labels=F)
dendroclr_pdo = give_dendrogram_generalised(as(compositions::clr(all_natgen[[which_natgen]][grepl('PDO', rownames(all_natgen[[which_natgen]])),]), 'matrix'), modify_labels=F)
dendroimput_pdo = give_dendrogram_generalised(impute(all_natgen[[which_natgen]][grepl('PDO', rownames(all_natgen[[which_natgen]])),], 1e-2), modify_labels=F)
dendroimputclr_pdo = give_dendrogram_generalised(as(compositions::clr(impute(all_natgen[[which_natgen]][grepl('PDO', rownames(all_natgen[[which_natgen]])),], 1e-2)), 'matrix'), modify_labels=F)
...
plot_rank = function(df_rank, nudge_scalar=1){
  ...
  ggplot(df_rank, aes(x=factor(Var1, as.character(Var1)[order(value)]),
    ...
    geom_label_repel(data=df_rank[c(T,F),], aes(x=factor(Var1, as.character(Var1)[order(value)]),
                                                y=value/2, label=labels), nudge_y=400*nudge_scalar)+
...
```{r, compare_amplification_generalised, echo=FALSE, eval=TRUE}
## scaling up to multiple genes (I have a problem with runtime)
give_ploidy_gene_scaling <- function(sample){
  
  # GRanges object with a row for each entry in subset_genes (sorted)
  gr_genes_of_interest_subset = as(geneInfo[which(geneInfo$genename %in% subset_genes),], 'GRanges')
  gr_genes_of_interest_subset = gr_genes_of_interest_subset[match(subset_genes, gr_genes_of_interest_subset$genename),]
  all(gr_genes_of_interest_subset$genename == subset_genes)
  ...
  r_scores = extractList(mcols(full_GRanges)$segVal, revmap_short)
  ...
  weights = sapply(1:length(gr_genes_of_interest_subset), function(gene_idx){
    ...
    ploidies_short = diploidifNA(sapply(1:length(genes_idx), function(j){
      ...
      r_scores[genes_idx,][j,][[1]][2]
    }))
    return(weighted_ploidy(ploidies_short,
                    widths_disjoint_short[genes_idx]))
  })
  names(weights) = subset_genes
  return(weights)
}

subsample_samples = all_ranges
all_ploidies_2 = mclapply(subsample_samples, give_ploidy_gene_scaling)
...
```{r, correlation_rnaseq_absoluteCN}
...
```{r, gene_ploidy_cosmic_plot, echo=FALSE}
names(ploidies)
names(ploidies[[1]])
...
```{r, genes_discrete_organoids_ampdel, echo=FALSE}

# Call TCGA signatures on BRITROC
...
OUT=file.path(BASE, "data/6_TCGA_Signatures_on_BRITROC")
dir.create(OUT, showWarnings = FALSE, recursive = TRUE)

# CN profiles and their correction
DATBRITROC=file.path(BASE, "data/validationdata/data_Geoff/data/britroc_absolute_copynumber.rds")
WIGGLE=0.1
IGNOREDELS=FALSE
CORES=7

# Extract features
PREPATH=file.path(BASE, "data/Geoffs_data/")
RMNORM=TRUE

# Calculate SxC matrix
INPUTMODELS=file.path(BASE, "data/3_Pancancer_Signatures/2_combined_mixmodels_merged_components.rds")
UNINFPRIOR="TRUE"

# Call exposures
SIGNATUREFILE=file.path(BASE, "results/Signature_Compendium_v5_Cosine-0.74_Signatures_newNames.rds")


## Load functions
source(file.path(BASE, "finalScripts/main_functions.R"))
...
qBR = readRDS(DATBRITROC)
allSamps = colnames(qBR)
lAllSegs = lapply(allSamps, function(thisSamp) { 

    dfSample = getSegTable(qBR[,which(allSamps==thisSamp)])
    dfSample$sample = thisSamp
    return(dfSample)
        
})
dfAllSegs = do.call(rbind, lAllSegs)
...
write.table(dfAllSegs, file.path(OUT, "0_BRITROC_absolute_CN.txt"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
saveRDS(dfAllSegs, file.path(OUT, "0_BRITROC_absolute_CN.rds") )
...
# Write txt and rds
write.table(dtSmooth, file.path(OUT, "1_BRITROC_absolute_CN_smoothed.txt"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
saveRDS(dtSmooth, file.path(OUT, "1_BRITROC_absolute_CN_smoothed.rds") )
...
saveRDS(brECNF, file.path(OUT, "2_BRITROC_ECNF.rds") )
...
saveRDS(object = allMats, file = file.path(OUT, "3_BRITROC_SxC_Matrix.rds"))
write.table(x = allMats, file = file.path(OUT, "3_BRITROC_SxC_Matrix.txt"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")

allMats = t( allMats )
saveRDS(object = allMats, file = file.path(OUT, "3_BRITROC_CxS_Matrix.rds"))
write.table(x = allMats, file = file.path(OUT, "3_BRITROC_CxS_Matrix.txt"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")
...
saveRDS(object = Hraw, file = file.path(OUT, "4_BRITROC_Exposures_to_TCGA_Signatures_raw.rds"))

H = t( apply(Hraw, 2, function(x) x/sum(x)) )
write.table(x = H, file = file.path(OUT, "4_BRITROC_Exposures_to_TCGA_Signatures.txt"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")
saveRDS(object = H, file = file.path(OUT, "4_BRITROC_Exposures_to_TCGA_Signatures.rds"))

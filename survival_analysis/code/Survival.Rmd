...
sapply(unique(PDS_PDO$sample), function(i){
  .x <- cor.test(PDS_PDO %>% filter(sample==i) %>%  select(auc_ll5.org) %>% unlist(),
           PDS_PDO %>% filter(sample==i) %>%  select(auc_ll5.sph) %>% unlist(),
           method = "pearson", conf.level = 0.95)
  cbind.data.frame(as.character(i), .x$estimate, .x$p.value)
})
